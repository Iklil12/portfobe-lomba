...
export function ManualCanvaManager() {
  ...
  const persistData = async (updatedProjects: { title: string; embedLink: string }[]) => {
    setIsSaving(true);
    const toastId = toast.loading('Menyimpan desain Canva...');
    try {
      ...
      if (res.ok) {
        await mutate();
        toast.success('Tersimpan otomatis!', { id: toastId });
      } else {
  ...
  const handleAdd = (e?: React.MouseEvent) => {
    ...
    if (projects.length >= 10) {
      toast.error('Maksimal 10 desain diperbolehkan.');
      return;
    }
    ...
    <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.01)] space-y-8 relative z-40">
              ...
              {editingIndex === idx ? (
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-extrabold text-slate-400 uppercase tracking-[0.2em] ml-1">Embed Link</label>
                      <input 
                        type="text" 
                ...
                >
                  <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 pr-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-black text-slate-900 tracking-tight truncate">{p.title || 'Untitled Design'}</p>
                      <p className="text-[10px] text-slate-400 font-medium truncate">{p.embedLink}</p>
                    </div>
                  </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-slate-50">
        <button type="button" onClick={handleAdd} className="w-full py-4 px-6 rounded-2xl bg-white border border-slate-200 text-slate-900 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-slate-50 hover:border-slate-300 transition-all flex items-center justify-center gap-3 shadow-sm active:scale-95">
          <i className="fas fa-plus-circle text-xs text-slate-400"></i> Add New Canva Project
        </button>
      </div>
    </div>
  );
